// Waveshare ESP32-S3-Knob-Touch-LCD-1.8 ST77916 init sequence.
//
// Arduino_GFX 1.5.0's built-in st77916_init_operations table does not drive
// this panel correctly — it comes up displaying diagonal noise. Two reasons:
//
//   1. Register 0xC4 (interface/lane control) is 0x47 in Arduino_GFX's table
//      but 0x74 on this board.
//   2. Arduino_GFX's table omits nothing, but Waveshare's esp_lcd path sends
//      MADCTL + COLMOD *before* the vendor sequence, and COLMOD (0x3A = 0x55,
//      16 bit/pixel) is what makes RGB565 pixel data decode correctly.
//
// This table is Waveshare's own 185-command sequence, mechanically converted
// from docs/demo-code/Arduino/examples/08_LVGL_Test/lcd_bsp.c in the
// Sandjab/Waveshare-Knob mirror into Arduino_GFX's batchOperation bytecode,
// with MADCTL and COLMOD prepended to match their driver's ordering.
//
// 1.5.0 takes no init-table constructor parameter (that arrived after this
// version), so the table is applied by overriding the protected tftInit().
#pragma once

#include <Arduino_GFX_Library.h>

static const uint8_t st77916_waveshare_init_operations[] = {
    BEGIN_WRITE,
    // Their esp_lcd driver writes MADCTL then COLMOD before the vendor
    // table; the vendor table itself contains neither. COLMOD is the
    // critical one: without it the panel stays in its default pixel
    // format and 16-bit data decodes as diagonal garbage.
    WRITE_C8_D8, 0x36, 0x00,   // MADCTL: no rotation, RGB order
    WRITE_C8_D8, 0x3A, 0x55,   // COLMOD: 16 bit/pixel
...
class Arduino_ST77916_Waveshare : public Arduino_ST77916 {
 public:
 ...
 protected:
  void tftInit() override {
    ...
    _bus->batchOperation(st77916_waveshare_init_operations,
                         sizeof(st77916_waveshare_init_operations));
  }
};
